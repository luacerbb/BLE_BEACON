...
///test houqz
uint8_t encode_scan_rep_packet(uint8_t *p_data)
{
		uint16_t  name_len = 0;
		uint8_t offset = 0;
		p_data[offset++] =  ADV_TYPE_LEN;
    ...
    sd_ble_gap_device_name_get(&p_data[offset + 2],
                                          &name_len);
	
		p_data[offset++] =  name_len + 1;
		p_data[offset++] =  BLE_GAP_AD_TYPE_COMPLETE_LOCAL_NAME;
...
///test get manu
uint8_t app_beacon_get_manuf_data(uint8_t *p_data)
{
	 uint8_t offset = 0;
	
    //p_data[offset++] =  ADV_DATA_BEACON_DATA_LEN + 1;                          
   // p_data[offset++] =  BLE_GAP_AD_TYPE_MANUFACTURER_SPECIFIC_DATA;
    //offset           += uint16_encode(m_beacon.manuf_id, &p_data[offset]);
    p_data[offset++] =  APP_DEVICE_TYPE;
...
void app_beacon_start(void)
{
    ...
    uint8_t adv_pdu[ADV_DATA_LEN];
		uint8_t rsp_pdu[ADV_DATA_LEN] = {0};;
		
		uint8_t rsp_len = 0;//encode_scan_rep_packet(rsp_pdu);
	
    uint8_t adv_len = encode_adv_packet(&adv_pdu[0]);
    
    err_code = sd_ble_gap_adv_data_set(adv_pdu, adv_len, rsp_pdu, rsp_len);
